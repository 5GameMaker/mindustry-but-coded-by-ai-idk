...
- `ChatFragment.visible()/draw()` 断线清理与淡出：`core/src/mindustry/ui/fragments/chat_fragment.rs` 已补 `ChatVisibilityUpdate`、`update_visibility_like_java()` 与 `tick_fade_like_java()`，按上游 `visible()` 在 `!net.active && messages.size > 0` 时清空消息并在输入打开时 `hide()`，可见性返回 `net.active && hudfrag.shown`；按上游 `draw()` 在隐藏状态下执行 `fadetime -= delta / 180f`，避免旧聊天消息永久遮挡视野；`desktop/src/lib.rs::update()` 已接入 `update_chat_visibility_like_java(game_state.is_game())` 与 `tick_chat_fade_like_java(1.0)`；新增 `visibility_update_clears_messages_and_hides_on_disconnect_like_java`、`fade_time_decays_only_when_hidden_like_java_draw`、`desktop_launcher_update_applies_chat_visibility_and_fade_like_java` 覆盖 core/desktop 闭环。
