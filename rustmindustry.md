...
- `cargo test -p mindustry-desktop desktop_launcher_campaign_route_ -- --nocapture`：通过，`23 passed; 0 failed`。
- Workspace crate：
  - `mindustry-core`
  - `mindustry-server`
  - `mindustry-desktop`
...
- 后续继续补真实 OpenGL 3D backend 执行、完整 numbered sector 选择/面板、hovered projection 特例图标/debug numbers 与 launch cutscene。
...
1. 继续补齐当前 UI 明确缺口：
   ...
   - 已补 planet surface hover 的首段 ray picking、hover label 投影、projection 全量可见 sector 层与 over-projection launcher/import/export/shield/attack 连线；后续继续补完整 numbered sector 选择/面板、sector 展开/选区实绘、launch cutscene。
...
2. 继续复核 graphics/g3d 行为深度：
   ...
   - `PlanetRenderer` 场景壳已完成，桌面 campaign route 已消费 `PlanetScenePlan` 并生成可见 preview primitives，且已补 surface hover ray picking、hover label 投影、projection 全量可见 sector 层与 over-projection launcher/import/export/shield/attack 连线；仍需 OpenGL backend 将 scene step 真实落成 3D draw。
