...
- 已新增跨 effect block 的轻量统一 runtime dispatcher：
  ...
  - `GameRuntimeUnitBlockState` / `unit_runtime_states` 已接入 `UnitFactory` 的 `progress/currentPlan/commandPos/commandId` 与 `UnitRepairTower` 的 `rotation`；同时通过 `distribution_runtime_states` 接入 `UnitCargoLoader readUnitId` 与 `UnitCargoUnloadPoint item/stale`，`UnitAssembler`/assembler module 的 unit list 与 module-link 状态仍需后续补专用 codec 后再接；
