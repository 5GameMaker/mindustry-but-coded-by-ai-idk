...
- 已新增跨 effect block 的轻量统一 runtime dispatcher：
  ...
  - `GameRuntimeLogicBlockState` / `logic_runtime_states` 已接入 logic building payload 的第一批 Java sidecar：`MessageBlock` 的 UTF message 文本、`SwitchBlock` revision 1 的 enabled bool、`LogicDisplay/TileableLogicDisplay` revision 1 的 `arc.math.Mat` 3x3 transform（bool + 9 个 float）、`MemoryBlock` 的 `memory.length + double[]`（读入容量超出时消费但截断，短缺时保留默认 0），以及 `CanvasBlock` 的 `data.length + byte[]`（长度不匹配时消费并保留默认像素数据）；`Processor` 的复杂 code/vars/TypeIO/wait payload 仍待后续分层承接；
