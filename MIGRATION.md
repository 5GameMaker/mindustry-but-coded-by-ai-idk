...
- 2026-05-26：`GameRuntimePayloadReadMode::{TopLevel,NestedExact}` 已区分顶层 terminal raw 保留与嵌套非末尾 exact 消费；当 `BuildPayload` 内部 block state 又包含 `PayloadConveyor / PayloadDeconstructor / PayloadVoid / UnitAssemblerModule` 这类原先 terminal reader 时，嵌套路径改用 exact payload-ref，不再 `read_to_end` 吞外层字段；已用 `UnitFactory.common -> BuildPayload(payload-conveyor -> BuildPayload(door)) -> factory fields` 回归测试锁定。
