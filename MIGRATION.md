...
- 已新增跨 effect block 的轻量统一 runtime dispatcher：
  ...
  - `GameRuntime` 已持有最小 owned building 集合并通过 `add_building(...)` 同步 `World` center tile 的 `block/build` 引用；`advance_owned_effect_blocks(...)` 可直接驱动 runtime 自有建筑集合，向 Java `Groups.build.update()` 入口继续靠拢（多格 footprint 与真实 EntityGroup/indexer 仍待接入）；
