...
- 2026-05-26：`PayloadUnloader` 已接入 Java `offloadSpeed` + `dumpAccumulate()` 的最小 item 输出运行态：`BuildingComp` 新增 transient `cdump/dump_accum`，`advance_owned_payload_loaders(...)` 在 unloader tick 结束后按 `offloadSpeed` 重复累积 `delta()`，并从 Java content item id 顺序中挑选库存，向同队 proximity 中的 `Storage/Core` 或 `ItemVoid` 执行最小 `acceptItem/handleItem` 搬运；新增 `GameRuntimePayloadLoaderFrameReport.dumped_items` 与 `game_runtime_payload_unloader_offloads_items_to_adjacent_storage_with_offload_speed`，锁定 payload 内 10 铜经 `timerLoad` 卸出 8 个后同 tick 按 `offloadSpeed=4` 输出 4 个到相邻 container。当前仍未接入完整 block-specific `acceptItem/handleItem/canDump` override、conveyor/router/item bridge/incinerator 等运输网络目标，以及 cdump 在复杂 proximity 变化下的全部 Java 语义。
- 2026-05-26：`PayloadUnloader` 已补上 Java `dumpLiquid(liquids.current())` 的最小液体外排运行态：unloader 从 payload 内卸载液体到自身后，会按 Java `dumpLiquid(..., 2f)` 的比例差与 `cdump` 邻接轮转，把当前液体单跳输出到同队 `LiquidBlock`（已用 `liquid-container` 验证）；新增 `GameRuntimePayloadLoaderFrameReport.dumped_liquid_events` 与 `game_runtime_payload_unloader_dumps_liquid_to_adjacent_liquid_container`，锁定 payload 内 80 water 经 `liquidsLoaded=40` 卸出后同 tick 向相邻 liquid-container 外排 20。当前仍未接入完整 `getLiquidDestination(...)` 链式路由、`LiquidJunction/ArmoredConduit` 特殊语义、所有 block-specific `acceptLiquid/canDumpLiquid` override 与真实 liquid graph 调度。
