...
- 已新增跨 effect block 的轻量统一 runtime dispatcher：
  ...
  - `GameRuntimeLogicBlockState` / `logic_runtime_states` 已接入 logic building payload 的第一批 Java sidecar：`MessageBlock` 的 UTF message 文本与 `SwitchBlock` revision 1 的 enabled bool；`Processor/Memory/Display/TileDisplay/Canvas` 的更复杂 code/vars/memory/transform/pixel payload 仍待后续逐类承接；
