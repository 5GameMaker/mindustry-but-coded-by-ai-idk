...
- 2026-05-26：`ItemBridge/phase-conveyor` 已接入 Java `ItemBridgeBuild.updateTile()/updateTransport()` 的最小普通 item runtime：新增 transient `item_bridge_transport_counters` 与 `GameRuntime::advance_owned_item_bridges(...)`，有效 link 会按 `transportCounter += edelta` 与 `transportTime` 批量从源桥转交到同队同 block 的 linked bridge，维护 target `incoming`、source `warmup/moved`；link 无效的输出端会复用真实 `dump_one_item_from_building(...)` 向相邻可接收端外排。`dump_target_accepts_item(...)` / `dump_item_to_target(...)` 也已允许 linked `ItemBridge` 接收来自源桥的 item，`advance_owned_payload_loaders(...)` 在 router/duct 后继续驱动 bridge tick。新增 `GameRuntimeItemBridgeFrameReport` 与 `game_runtime_item_bridge_transfers_and_output_dumps_to_real_receiver`，锁定 source phase-conveyor -> linked phase-conveyor -> item-void 的端到端流转。当前仍未接入 `BufferedItemBridge/bridge-conveyor`、`Junction`、`DuctBridge` 的专用 Java runtime，`ItemBridge` 的 `checkIncoming` 周期窗口、`timeSpeed/time` 动画、完整 `checkAccept/checkDump` 边向兼容、液体桥接与复杂双向链仍待迁移。
...
- 2026-05-26 item bridge 最小 runtime 验证：`cargo test game_runtime_item_bridge_transfers_and_output_dumps_to_real_receiver --lib` 通过 1/1；`cargo test forwards_unloaded_item_to_real_receiver --lib` 通过 4/4；`cargo test game_runtime_payload_unloader --lib` 通过 12/12；`rustfmt core/src/mindustry/core/game_runtime.rs` 与 `git diff --check` 通过。新增 `GameRuntime::advance_owned_item_bridges(...)`、`item_bridge_transport_counters` 与 `bridge_forwarded_items`，覆盖 linked `phase-conveyor` 源桥按 `transportTime` 转交到输出桥，输出桥 link invalid 后向相邻 `item-void` 外排。
