...
- `DesktopGraphicsLiveBackendRenderTargetSink` 已记录每个 render pass 的 target `Begin/End` 生命周期，`RenderTarget::Screen` 对应默认 backbuffer，`Texture/Buffer` 对应后续 OpenGL FBO/离屏纹理路径；`RenderPass::resolve_target` 会显式生成 `Resolve` 事件，禁止对所有非 Screen target 自动推导回填。
...
- 已跑验证：
  ...
  - `git diff --check`
- 仍未完成：
  - camera shake runtime 本地事件已在 `12.275` 下沉到 desktop pending/state；真实 camera backend 的随机 offset 应用仍未接；
  - `UnitComp.destroy()`、flying wreck、其他 effect shake 调用仍未全面接入；
  - 多个 shake event 的 backend 合并、随机方向 offset 与配置读取还需继续对照 Java/Arc 实现。

### 12.275 UnitSafeDeath camera shake 下沉到 desktop render seam

- 2026-05-28：对照 Java `Effect.shake(...) -> Vars.renderer.shake(...) -> Renderer.shake(...)`，把 `UnitSafeDeathCallPacket` 产生的 runtime camera shake 事件接入 desktop launcher render seam，不再只停留在 core runtime 队列。
- Java 依据：
  - `Effect.shake(float intensity, float duration, float x, float y)` 按 `Core.camera.position.dst(x, y)` 和 `shakeFalloff` 计算强度衰减；
  - `Renderer.shake(...)` 使用 `max` 合并 `shakeIntensity` / `shakeTime`，并设置 `shakeReduction = shakeIntensity / shakeTime`；
  - 每帧按 `screenshake / 4f * 0.75f` 得到最大随机 offset 强度，再随 `Time.delta` 衰减。
- Rust 新增/变化：
  - `desktop/src/lib.rs`
    - 新增 `DesktopCameraShakeState { intensity, time, reduction }` 与 `DesktopCameraShakeFrame { max_offset, remaining_intensity, remaining_time }`；
    - `DesktopLauncher` 新增 `pending_camera_shake_events`、`camera_shake_state`、`last_camera_shake_frame`；
    - `DesktopLauncher::update()` 会从 `runtime.client_local_camera_shake_events` 转移事件，用 `shake_intensity(...)` 做 Java falloff 语义的强度解析，并按默认 `screenshake=4` tick 一帧；
    - 新增 `sync_local_camera_shake_events_for_render(...)`、`tick_camera_shake_for_render(...)`、`drain_camera_shake_events_for_render(...)`，为后续真实 desktop camera/backend 接入提供明确入口；
    - safe death desktop 测试更新为验证 runtime 事件已被下沉到 desktop pending/state；
    - 新增 `desktop_launcher_resolves_camera_shake_events_for_render_like_java_effect_shake` 覆盖距离衰减、max-offset、衰减状态和 pending drain。
- 已跑验证：
  - `cargo test -p mindustry-desktop desktop_launcher_syncs_unit_safe_death_packet_to_runtime_remove_effect`
  ...
  - `cargo check -p mindustry-desktop`
- 仍未完成：
  - 真实 renderer/backend 尚未把 `last_camera_shake_frame.max_offset` 转为 Java 风格随机方向 `camShakeOffset` 并应用到 camera position；
...
- 仍未完成：
  ...
  - `UnitDestroyCallPacket` 已在 `12.280` 接入 `UnitComp.destroy()` 的动态爆炸、死亡音效、死亡震动主副作用；
  - 当前总体迁移仍约 10%~11%，远未可玩。

### 12.280 UnitDestroy 主死亡副作用：dynamicExplosion / deathSound / deathShake

- 2026-05-28：继续对照 Java `UnitComp.destroy()`，把 Rust 客户端收到 `UnitDestroyCallPacket` 时的主死亡副作用从“只腿部碎裂+移除”推进到动态爆炸、死亡音效、死亡镜头震动也进入真实 runtime/desktop seam。
- Java 依据：
  - `Damage.dynamicExplosion(..., type.deathExplosionEffect, 0f)` / core-spawn 分支 `type.deathExplosionEffect.at(...)`；
...
- Rust 新增/变化：
  - `core/src/mindustry/core/game_runtime.rs`
    ...
    - `apply_client_unit_destroy_packet(...)` 现在在移除前排入：
      - `death_explosion_effect` 对应 `EffectCallPacket2`，默认 `dynamicExplosion`；
      - Java destroy 公式的 `GameRuntimeClientCameraShakeEvent`；
      - 通过 `UnitType::pure_init_plan().death_sound` 解析出的死亡音效；
      - 原有 `legDestroy` 仍保留；
    - 更新 core `UnitDestroy` / `UnitDeath` 测试，覆盖 dynamicExplosion、legDestroy、deathSound、deathShake。
  - `desktop/src/lib.rs`
    - 更新 `desktop_launcher_syncs_unit_destroy_packet_to_leg_destroy_effects` 和 `desktop_launcher_syncs_multiple_unit_lifecycle_packets_in_one_update`，验证 lifecycle -> runtime -> local effect/materialize + pending sound/camera seam 的完整路径。
- 已跑验证：
  - `cargo test -p mindustry-core game_runtime_applies_client_unit_destroy_packet_to_legged_unit_effects`
  - `cargo test -p mindustry-core game_runtime_applies_client_unit_death_packet_like_java_killed`
  - `cargo test -p mindustry-desktop desktop_launcher_syncs_unit_destroy_packet_to_leg_destroy_effects`
  ...
  - `cargo check -p mindustry-desktop`
- 仍未完成：
  - `Damage.dynamicExplosion(...)` 的 lightning/fire/wave damage/shockwave 仍只接了爆炸 effect 主 seam；
  - `createScorch -> Effect.scorch(...)` 已在 `12.281` 接入本地 decal seam；wreck decal、weapon `shootOnDeath`、ability `death(...)`、`type.killed(...)`、event bus、suicide trigger 尚未完成；
  - 真实 audio/camera backend 仍需继续接入；
  - 当前总体迁移仍约 10%~11%，远未可玩。

### 12.281 UnitDestroy createScorch 本地 decal seam

- 2026-05-28：继续对照 Java `UnitComp.destroy()` 的 `if(type.createScorch) Effect.scorch(...)`，在 Rust runtime 中为单位销毁创建本地 scorch decal。
- Java 依据：
  - `Effect.scorch(x, y, (int)(hitSize / 5));`
  - `size = Mathf.clamp(size, 0, 9);`
  - `Core.atlas.find("scorch-" + size + "-" + Mathf.random(2));`
  - `decal(region, x, y, Mathf.random(4) * 90, 3600, Pal.rubble);`
- Rust 新增/变化：
  - `core/src/mindustry/core/game_runtime.rs`
    - `GameRuntime` 新增 `next_client_local_decal_id`，用负 id 为本地 decal 分配实体槽；
    - reset/clear 路径重置 `next_client_local_decal_id`；
    - `queue_client_unit_destroy_side_effects(...)` 在 `create_scorch` 为 true 时创建本地 `DecalComp`：
      - region：`scorch-{size}-{variant}`；
      - lifetime：`3600`；
      - color：`Pal.rubble` 对应 `0x1c1817ff`；
      - rotation：当前为 deterministic variant seam，后续接真实随机；
    - 更新 core destroy 测试验证 scorch decal 位置、region、rotation、lifetime 和本地负 id。
  - `desktop/src/lib.rs`
    - 更新 unit destroy 和多 lifecycle desktop 测试，验证 NetClient lifecycle -> runtime 后本地 scorch decal 已进入 `client_decal_snapshot_entities`。
- 已跑验证：
  - `cargo test -p mindustry-core game_runtime_applies_client_unit_destroy_packet_to_legged_unit_effects`
  - `cargo test -p mindustry-desktop desktop_launcher_syncs_unit_destroy_packet_to_leg_destroy_effects`
  - `cargo test -p mindustry-desktop desktop_launcher_syncs_multiple_unit_lifecycle_packets_in_one_update`
...
## 12.455 Shader source loader 接入
