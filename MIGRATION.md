...
- `DesktopGraphicsLiveBackendRenderTargetSink` 已记录每个 render pass 的 target `Begin/End` 生命周期，`RenderTarget::Screen` 对应默认 backbuffer，`Texture/Buffer` 对应后续 OpenGL FBO/离屏纹理路径；真正的 `Blit`/回填事件仍待补齐。
