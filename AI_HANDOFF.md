...
- 仍未完成：Java 共享 `ItemModule` 引用的真正内存级等价、真实 `StorageBlock.incinerateEffect(...)` 视觉效果、更多 owned runtime tick 纳入 server update 主循环、普通 core 新建放置与升级 FX/Event、真实 player spawn、完整 UI/renderer 行为。
