...
- 已接入：`GameRuntimeNetworkContext` 对齐 Java `CoreBuild.handleItem()` 的 `net.server() || !net.active()` 条件；默认离线仍按权威端写 core items/campaign，active client 只增加 stats，不直接写 core items，也不重复写 campaign delta。
...
- 仍未完成：Java 共享 `ItemModule` 引用的真正内存级等价、真实 `StorageBlock.incinerateEffect(...)` 视觉效果、net role 自动注入所有 runtime tick、core placement/upgrade 全流程、team core registry、完整 UI/renderer 行为。
