...
## 最新闭环：补齐 RulesJsonPatch 区域限制与背景字段组

- 当前总体迁移完成度：约 **99.998%**，仍未达到完整可玩。
- 本轮实现：
  - `core/src/mindustry/game/rules.rs`
    - `RulesJsonPatch` 新增区域限制字段：`limitMapArea`、`limitX`、`limitY`、`limitWidth`、`limitHeight`、`disableOutsideArea`；
    - `RulesJsonPatch` 新增背景字段：`customBackgroundCallback`、`backgroundTexture`、`backgroundSpeed`、`backgroundScl`、`backgroundOffsetX`、`backgroundOffsetY`；
    - `customBackgroundCallback` / `backgroundTexture` 支持字符串写入与 `null` 清空；
    - 新增 `rules_apply_json_str_supports_limit_area_fields` 与 `rules_apply_json_str_supports_background_fields`。
- 额外发现：
  - Java `Rules.java` 还有 `planetBackground: PlanetParams`；Rust 当前没有等价字段/类型，本轮未误降级为字符串，后续需单独补对象型等价结构或明确 runtime 替代。
- 已验证/本轮收口验证：
  - `cargo fmt --all`
...
- 下一步优先：
  1. Net：在 `tests/src/lib.rs` 补 `ServerLauncher + DesktopLauncher` 高层真实 join smoke，复用 `free_local_port()` 与 `pump_real_server_desktop_until(...)`；
  2. Rules：评估并补齐 `planetBackground: PlanetParams`；
...
- 本轮并行审查覆盖 UI、net、world/save、content 与文档进度；已将关键接入点写回 `MIGRATION.md`。
- 本轮实现：
  - `core/src/mindustry/content/mod.rs`
    - `ContentCatalog::load_base_content()` 的 loader 调用顺序对齐 Java `ContentLoader.createBaseContent()`；
    - 保留 `unit_stances::load(&items)` 与 `blocks::load(&items, &liquids)` 依赖注入；
    - 新增 Java 基础内容加载顺序回归常量/测试。
  - `core/src/mindustry/core/version.rs`
    - 新增 Java v157.4 握手基线常量：build `157`、revision `4`、type `official`；
    - 新增 `DEFAULT_HANDSHAKE_BUILD` / `DEFAULT_HANDSHAKE_TYPE`。
  - `core/src/mindustry/core/net_client.rs`
    - 默认 `ConnectPacket` 版本/type 改为 v157.4 基线。
  - `core/src/mindustry/core/net_server.rs`
    - 默认 `ConnectPacketValidationContext` 改为 v157.4 基线；
    - 版本拒绝测试调整为 `158 => ServerOutdated`、`156 => ClientOutdated`。
- 已验证/本轮收口验证：
  ...
  - `cargo check -p mindustry-core --lib`
- 下一步优先：
  1. Net：真实 Java↔Rust 握手/世界流 smoke，继续复核 `packets.rs` 与 v157.4 的全量协议差异；
  2. Content：补 Block/UnlockableContent 的 stats/bars/bundle/icon 生命周期桥接；
  3. World/save：补 `SaveSnapshot` 统一应用入口与 `RulesJsonPatch` 字段覆盖；
  4. UI：补 `desktop_workshop_enabled` 启动态联动、Load slot decoration、Database/ContentInfo UI-stat-bar 细节。
...
- 本轮实现：
  - `desktop/src/lib.rs`
    ...
    - `push_load_game_route_page(...)` 改用 `load_game_slot_render_lines(...)`，命中时复用卡片标题、地图、模式/波次、autosave、playtime、date 文本和 autosave checked 状态；
...
## 最新闭环：缓存 Settings pref specs runtime 投影

- 当前总体迁移完成度：约 **99.989%**，仍未达到完整可玩。
- 用户当前重点：前端/UI 所有主菜单与子菜单继续还原 Java 原版，同时必须优化“帧数极其极其低下”的前端性能问题。
- 本轮实现：
  - `desktop/src/lib.rs`
    ...
    - `settings_pref_widget_specs_for_current_runtime(table)` 以 table、mobile、iOS、Steam、prerelease lobby gating、macOS、shader availability 为 key；
    - 同一 Settings 子页稳定帧复用可见 spec 投影，减少每帧 Settings 条件判断和 Vec 重建；
...
- 本轮实现：
  - `desktop/src/lib.rs`
    - 新增 `opengl_backend_sprite_mesh_layer_batch_bits(...)`，仅对 `Layer::END_PIXELED..=Layer::MAX` 的前端 UI 层做 `0.001` 粒度量化；
    - `opengl_backend_sprite_mesh_batch_key(...)` 改用量化后的 UI layer bits，合并 Settings/Join/Load/Mods 中大量 `+ index * 0.0001` 微层导致的碎批；
    - `Layer::BLOCK`、`Layer::EFFECT` 等世界层仍走精确 `to_bits()`，保持大层 draw-call 排序与既有世界渲染语义；
    - 新增 `desktop_graphics_opengl_backend_batches_ui_micro_layers_into_coarse_bins_for_stable_fps`，锁住前端微层合批与相邻 UI z-band 分离。
- 已验证/本轮收口验证：
  - `cargo fmt --all`
...
## 最新闭环：缓存 Join/Mods 单行省略文本降低稳定帧开销

- 当前总体迁移完成度：约 **99.985%**，仍未达到完整可玩。
- 用户当前重点：前端/UI 所有主菜单与子菜单继续还原 Java 原版，同时必须优化“帧数极其极其低下”的前端性能问题。
- 本轮实现：
  - `desktop/src/lib.rs`
    - 对照 Java `JoinDialog`/`ModsDialog` 的 Scene2D 控件重建节奏，给 Rust 侧 Join/Mods 共用的单行 ellipsis 文本投影加小型稳定 cache；
    - `desktop_join_single_line_ellipsis_like_java(...)` 保留 markup tag 零宽、换行替换为空格、ASCII/非 ASCII 估宽和末尾 `...` 行为；
    - cache key 覆盖原文、`max_width`、`font_size`，同 key 稳定帧直接复用截断字符串，减少服务器/模组卡片逐字符扫描；
    - 新增 `desktop_join_single_line_ellipsis_reuses_cached_projection_for_stable_fps`，用 sentinel 锁住二次调用命中 cache。
- 已验证/本轮收口验证：
  - `cargo fmt --all`
...
- 下一步优先级：
  1. FPS：OpenGL UI layer/batch 量化或 route-page 主体缓存，降低 draw call 和 command 重建；
  2. UI：Join 服务器卡片 header/body、Mods Browser/详情弹窗、Settings/Load 子菜单细节继续对照 Java；
  3. 可玩性：继续把 UI 与 world/runtime/net 联动补齐，不能做成孤立模块。
...
- 本轮实现：
  - `desktop/src/lib.rs`
    - 对照 Java `SettingsMenuDialog.rebuildMenu()` 的 `menu.defaults().size(300f, 60f)`、`marginLeft(8f)` 和 `iconMed`；
    - 实现已保持 300×60、icon 32×32、marginLeft 8、label gap 18，本轮补强 Settings 主菜单回归断言；
    - 锁住 `Icon.settings` 通过 icon font 绘制在 icon cell center，label 从 `icon center + iconMed/2 + 18` 开始，防止后续 UI 修复破坏 Java `Styles.flatt` 节奏。
- 已验证/本轮收口验证：
  - `cargo fmt --all`
  - `cargo test -p mindustry-desktop desktop_launcher_settings_main_page_renders_upstream_menu_buttons --lib -- --nocapture`
- 下一步优先级：
  1. UI：继续 LoadGame 搜索行/模式筛选 chip、Join 服务器卡片、Mods/Database 子菜单；
...
- 本轮实现：
  - `desktop/src/lib.rs`
    - `push_menu_logo_and_version_chrome(...)` 改为先按 Java `fx = (int)(width / 2f)` 得到 logo 中心，再由中心推出 sprite rect；
    - version 文本 x 复用同一个 `logo_center_x`；
    - 去掉 version y 的 Rust-only viewport clamp，保持 Java `fy - logoh/2 - Scl.scl(2f)` 语义；
    - 新增奇数 viewport 宽度 + fractional scaled logo region 回归，锁住亚像素/1px 漂移；
    - 旧 logo overlay 测试的背景断言改为接受 Java world darkness 或当前 bucketed synthetic background，避免和 logo 位置无关的背景路径误报。
- 已验证/本轮收口验证：
  - `cargo fmt -- desktop/src/lib.rs`
...
- 本轮实现：
  - `desktop/src/lib.rs`
    - `DesktopRealFontTextLayoutGlyph` 不再只保存原始 glyph，而是缓存 glyph uv、缩放后的 width/height/offset/advance 和 atlas region 尺寸；
    - `DesktopRealFontTextLayout` 缓存 `glyph_count`，多层 shadow/outline/foreground pass 生成 quads 时不再反复统计 glyph 数量；
    - `opengl_backend_text_real_font_quads_from_layout(...)` 直接使用预计算 metrics，减少每个字形每层重复乘 scale、ceil 和 advance 计算。
- 已验证/本轮收口验证：
  - `cargo fmt -- desktop/src/lib.rs`
  - `cargo test -p mindustry-desktop desktop_graphics_opengl_backend_real_font_ --lib -- --nocapture`
  - `cargo test -p mindustry-desktop desktop_graphics_opengl_backend_markup_colors_real_font_foreground_like_java --lib -- --nocapture`
...
## 最新闭环：去掉 black6 面板 fallback 的 Rust-only 描边

- 当前总体迁移完成度：约 **99.98%**，仍未达到完整可玩。
- 用户当前重点：前端/UI 还原继续优先，同时持续处理“帧数极其极其低下”的性能问题。
- 本轮实现：
  - `core/src/mindustry/graphics/menu_renderer.rs`
    - 对照 Java `Styles.black6 = whiteui.tint(0f, 0f, 0f, 0.6f)`，去掉 `menu_push_black6_panel(...)` fallback 路径里额外的 `StrokeRect`；
    - 扩展 `menu_ui_plan_desktop_draws_black6_main_and_submenu_panels_like_java_tables`，锁住主/子菜单 `black6` panel 不能额外画 Rust-only border。
  - `README.md`
    - 迁移进度更新到 **99.98%**。
  - `MIGRATION.md`
    - 新增 `1223. 去掉 black6 面板 fallback 的 Rust-only 描边`。
- 已验证/本轮收口验证：
  ...
  - `cargo build -p mindustry-desktop --release`
- 下一步优先级：
  1. UI：补锁 desktop 主按钮 icon/label inset、submenu sibling table 230f/70f 回归测试，并继续收口顶部/右侧 chrome widget skin；
  2. FPS：继续做文本 layout 跨帧缓存与 text layer/batch 收敛；
  3. 可玩性：继续把 UI 与 world/runtime/net 联动补齐，不能做成孤立模块。
...
- 本轮实现：
  - `desktop/src/lib.rs`
    - 新增 `DesktopContentIconRuntimeRegistryCache` 与 frame validation token；
    - `DesktopLauncher` 新增 content icon runtime registry 缓存、rebuild 计数与 active frame token；
    - `format_icons_like_java(...)` 通过 cached registry 解析 `:icon:` token；
    - `campaign_sector_content_icon_source_like_java(...)` 复用 cached registry；
    - 新增 `desktop_launcher_reuses_content_icon_runtime_registry_within_menu_frame`。
  - `README.md`
    - 迁移进度更新到 **99.94%**。
  - `MIGRATION.md`
    - 新增 `1217. 缓存 content icon runtime registry 帧内复用`。
- 已验证/本轮收口验证：
  - `cargo fmt`
...
- 本轮实现：
  - `desktop/src/lib.rs`
    ...
    - `DesktopLauncher` 新增 `external_bundle_active_frame_index`、`external_bundle_last_frame_validation` 与测试用 validation 计数；
    - `refresh_external_bundle_cache_for_locale(...)` 在同一菜单帧同一 `data_dir + locale` 内只做一次 external bundle 文件系统校验；
    - 保留跨帧 revalidate 与非帧内读取的热更新语义；
    - 新增 `desktop_launcher_validates_external_bundle_family_once_per_menu_frame`。
  - `README.md`
    - 迁移进度更新到 **99.92%**。
  - `MIGRATION.md`
    - 新增 `1215. 缓存主菜单外置 bundle 帧内文件校验`。
- 已验证/本轮收口验证：
  - `cargo fmt`
...
## 最新闭环：将主菜单 Mac notch 偏移改为运行时高度

- 当前总体迁移完成度：约 **99.91%**，仍未达到完整可玩。
- 用户当前重点：前端/UI 还原继续优先，同时持续处理“帧数极其极其低下”的性能问题。
- 本轮实现：
  - `desktop/src/lib.rs`
    - 新增 `DESKTOP_MENU_MAC_NOTCH_HEIGHT_LIKE_JAVA = 32.0`；
    - `DesktopLauncher` 新增 `menu_macnotch_height`，默认对齐 Java `Vars.macNotchHeight`；
    - `DesktopMenuChromeLayoutOptions` 增加 `macnotch_height`；
    - logo/version 位置使用 runtime notch height，不再在公式里硬编码 32；
    - `graphics.macnotch` 设置副作用同步更新 `menu_macnotch_enabled`；
    - 更新 logo notch 测试，使用 48px runtime notch height 锁住动态高度。
  - `README.md`
    - 迁移进度更新到 **99.91%**。
  - `MIGRATION.md`
    - 新增 `1214. 将主菜单 Mac notch 偏移改为运行时高度`。
- 已验证/本轮收口验证：
  - `cargo fmt`
...
- 本轮实现：
  - `desktop/src/lib.rs`
    - `campaign_intro_select_dialog_rect(...)` 改成 viewport 居中的 BaseDialog 叠层，不再被右侧 route shell 小 panel 限死；
    - `campaign_intro_planet_button_rect(...)` 改为 Java desktop 320 方形目标尺寸；
    - `push_campaign_intro_planet_button(...)` 删除圆形几何占位，改为绘制 `sprites/planets/serpulo.png` / `sprites/planets/erekir.png`；
    - campaign select `@ok` 改为 `300x64`，Serpulo rework notice `@ok` 改为 `280x64`；
    - 扩展 Campaign select / rework notice 测试，锁住方形按钮、planet sprite、OK 高度与点击命中。
  - `README.md`
    - 迁移进度更新到 **99.89%**。
  - `MIGRATION.md`
    - 新增 `1212. 对齐 Campaign 首次选星弹窗 planet 图卡与 OK 按钮高度`。
- 已验证/本轮收口验证：
  - `cargo fmt`
...
## 最新闭环：清空 OpenGL recording driver 历史命令避免跨帧累积

- 当前总体迁移完成度：约 **99.88%**，仍未达到完整可玩。
- 用户当前重点：前端/UI 还原继续优先，同时持续处理“帧数极其极其低下”的性能问题。
- 本轮实现：
  - `desktop/src/lib.rs`
    - `DesktopGraphicsRecordingOpenGlBackendDriver` 新增 `clear_frame_commands()`；
    - `DesktopGraphicsNullOpenGlBackendRuntime::submit_resolving_executor(...)` 每帧提交前清空 recording commands；
    - renderer 测试改为检查当前帧记录，不再默认历史命令跨帧累积；
    - 新增 `desktop_graphics_opengl_backend_runtime_keeps_recording_driver_to_last_frame`，锁住只保留最近一帧命令。
  - `desktop/src/main.rs`
    - `DesktopNativeOpenGlRuntime::submit_resolving_executor(...)` 在构造 native driver 前清空 recording commands；
    - texture/buffer/VAO/shader/draw-state cache 仍跨帧保留。
  - `README.md`
    - 迁移进度更新到 **99.88%**。
  - `MIGRATION.md`
    - 新增 `1211. 清空 OpenGL recording driver 历史命令避免跨帧累积`。
- 已验证/本轮收口验证：
  - `cargo fmt`
...
## 最新闭环：对齐 BE check 默认标签纯白颜色

- 当前总体迁移完成度：约 **99.87%**，仍未达到完整可玩。
- 用户当前重点：前端/UI 还原继续优先，同时持续处理“帧数极其极其低下”的性能问题。
- 本轮实现：
  - `desktop/src/lib.rs`
    - `menu_becheck_label_color()` 无更新分支对齐 Java `MenuFragment` 的 `Color.white`；
    - 保留检测到更新时 white/accent 呼吸插值；
    - 扩展 `desktop_launcher_menu_renders_desktop_and_discord_chrome_buttons`，锁住 `@be.check` 默认纯白行为。
  - `README.md`
    - 迁移进度更新到 **99.87%**。
  - `MIGRATION.md`
    - 新增 `1210. 对齐 BE check 默认标签纯白颜色`。
- 已验证/本轮收口验证：
  - `cargo fmt`
...
- 下一步优先级：
  ...
  2. UI：继续收口主菜单 chrome、Settings 主菜单容器、ModsBrowser/Host/Schematics/Campaign/Planet；
  3. 继续保证模块接入整体 runtime/render/backend 主链路，不允许做成孤立模块。
...
- 本轮实现：
  - `desktop/src/lib.rs`
    - `DesktopFontGlyphUploadPlanCacheKey` 从克隆字体源 Vec + 构造 content icon visibility Vec，改为 `(len,digest)` 轻量 key；
    - 新增流式 FNV digest helper，覆盖字体源、content icon registry、atlas resolved 状态和外部 font seed；
    - 保持字体源变化、locale 重启加载、content icon/atlas 变化、外部 bundle seed 变化时仍会失效。
  - `README.md`
    - 迁移进度更新到 **99.78%**。
  - `MIGRATION.md`
    - 新增 `1201. 轻量化字体上传计划缓存 key 降低稳定帧分配`。
- 已验证：
  - `cargo fmt`
  ...
  - `cargo build -p mindustry-desktop --release`
- 下一步优先级：
  1. FPS：缓存/轻量化 `format_icons_like_java`、`localize_bundle_markup_text`、Join 可见窗口分配；
  2. UI：继续补 Join/Settings/Load/Host/Mods/Schematics 子菜单与 Java Scene2D 视觉差；
  3. 资源：继续直接复用上游 assets、fonts、bundles、icons、sprites/ui。
...
- 本轮实现：
  - `desktop/src/lib.rs`
    ...
    - `join_add_dialog_text_rect(...)` 使用 Java `size(320f, 54f)`；
    - `join_add_dialog_button_rect(...)` 使用 Java `size(140f, 60f).pad(4f)`，两按钮间距修正为 `8f`；
    - 扩展 `desktop_launcher_join_route_renders_server_browser_skeleton` 锁住输入框、按钮和间距。
  - `README.md`
    - 迁移进度更新到 **99.76%**。
  - `MIGRATION.md`
    - 新增 `1199. 对齐 Join Add Server 弹窗按钮间距`。
- 已验证：
  - `cargo fmt`
  - `cargo test -p mindustry-desktop desktop_launcher_join_route_renders_server_browser_skeleton -- --nocapture`
  ...
  - `cargo build -p mindustry-desktop --release`
- 下一步优先级：
  1. FPS：查稳定帧每帧分配、文本测量、资源/字体缓存、Join/Settings/Load 子菜单热点；
  2. UI：继续补 Join community host 正文流式排版、group header 按钮 skin、Add Server 弹窗 shell；
  3. 子菜单：Settings、Load/Save、Host、Mods、Schematics 继续贴 Java 原版。
...
- 本轮实现：
  - `desktop/src/lib.rs`
    - 对照 `D:/MDT/mindustry-upstream-v157.4/core/src/mindustry/ui/dialogs/JoinDialog.java` 的 `columns()` / `section(...)` / `refreshCommunity()`；
    - Join community host rows 从旧固定单列改为 Java-like 多列：1280x720 下 2 列 × 3 行首屏，列间距按 5f；
    - render、hit-test、tooltip 统一使用 `visible_join_community_entry_window_for_panel(...)`，滚动一步按 `columns()` 推进，避免绘制与命中窗口不同步；
    - community visible cache key 从每帧全量 group/host fingerprint 改为版本号，feed/加载/隐藏/清空组时主动失效；
    - group header label width 增加 `(locale, group_name)` 缓存，减少稳定帧字体 atlas/字形度量开销。
  - `README.md`
    - 迁移进度更新到 **99.74%**。
  - `MIGRATION.md`
    - 新增 `1197. 对齐 Join 社区多列布局并降低稳定帧开销`。
- 已验证：
  - `cargo fmt`
  ...
  - `cargo build -p mindustry-desktop --release`
- 下一步优先级：
  1. 继续低 FPS：saved-server snapshot 缓存、Join 静态文本缓存、font glyph upload key generation 化；
  2. Join UI 还原：community host 正文流式排版、group header 按钮 skin、Add Server 弹窗 shell；
  3. 继续 UI 还原：Settings、Load/Save、Host、Mods、Schematics 子菜单贴 Java 原版。
...
- 本轮实现：
  - `desktop/src/lib.rs`
    - 新增 `DesktopJoinCommunityVisibleCache`，缓存 Join community 可见 group indices 与展开后的 `(group, host)` entry indices；
    - `visible_join_community_groups()` / `visible_join_community_entries()` 保持原返回接口，但内部复用缓存，滚动上限改走 cached entry count；
    - cache key 覆盖提交后的 search、show hidden、hidden settings、group/host 搜索字段 fingerprint，保持 Java `refreshCommunity()` 提交搜索语义；
    - 删除已被缓存路径取代的旧 `DesktopJoinCommunityGroup::matches_query`；
    - 新增 `desktop_launcher_caches_visible_join_community_entries_between_stable_queries`。
  - `README.md`
    - 迁移进度更新到 **99.72%**。
  - `MIGRATION.md`
    - 新增 `1195. 缓存 Join 社区可见列表降低联机菜单帧开销`。
- 已验证：
  - `cargo fmt`
  - `cargo test -p mindustry-desktop desktop_launcher_caches_visible_join_community_entries_between_stable_queries -- --nocapture`
  - `cargo test -p mindustry-desktop join_route -- --nocapture`
...
- 本轮实现：
  - `desktop/src/lib.rs`
    ...
    - 菜单帧/世界帧通过 `Arc::clone` 共享稳定 atlas，减少每帧大对象深拷贝；
    - atlas 合并、mod 图标合并、预览贴图合并和 `graphics.linear` 切换改为 `Arc::make_mut`，只在写入时 copy-on-write；
    - 修正相关测试 fixture，并把默认 atlas 测试从旧虚拟直连 PNG 断言更新为 Java packed atlas/page-specific 断言。
  - `README.md`
    - 迁移进度更新到 **99.68%**。
  - `MIGRATION.md`
    - 新增 `1191. 避免菜单帧深拷贝贴图图集`。
- 已验证：
  - `cargo fmt`
  - `cargo test -p mindustry-desktop desktop_launcher_reuses_cached_font_glyph_upload_plan_for_stable_frames -- --nocapture`
  - `cargo test -p mindustry-desktop desktop_launcher_menu_graphics_frame_for_surface_uses_single_ui_command_build -- --nocapture`
  ...
  - `cargo build -p mindustry-desktop --release`
- 下一步优先级：
  1. 继续优化低 FPS：缓存外置 bundle/locale 读取，排查 font glyph upload plan cache key churn；
  2. 检查 native event loop 是否存在 vsync/fpscap/WaitUntil 双节流；
  3. 继续 UI 还原：Host/Join/Load/Settings/Mods 所有子菜单保持 Java 原版表现，不只做主菜单。
...
- 本轮对照：
  - `D:/MDT/mindustry-upstream-v157.4/core/src/mindustry/ui/dialogs/LanguageDialog.java`
  - Java 每行是 `TextButton(getDisplayName(loc), Styles.flatTogglet)`，并以 `size(400f, 50f)` 加入 ScrollPane。
- 本轮实现：
  - `desktop/src/lib.rs`
    - 新增 `SETTINGS_LANGUAGE_LABEL_FONT_SIZE_LIKE_JAVA`；
    - LanguageDialog 渲染改为读取 `LanguageDialogRow.button_style` 和 `LanguageDialogRow.font`，不再在渲染层硬编码语言行按钮 style / 字体；
    - 扩展 `desktop_launcher_language_dialog_uses_fonts_def_shadow_not_outline_like_java`，锁住 English 行的 Java 默认字号、默认字体、非 outline。
  - `README.md`
    - 迁移进度更新到 **99.66%**。
  - `MIGRATION.md`
    - 新增 `1189. 接入 LanguageDialog 行模型字体和样式`。
- 已验证：
  - `cargo fmt`
...
- 下一步优先级：
  1. 继续补 Settings/Language 启动 locale、外置 bundle 运行时入口和字体装载入口；
  2. 继续补 Host/Join/Mods 子菜单交互壳层；
  3. 继续扫 raw key / 英文 fallback / 图标 token 泄漏。

## 最新闭环：修正 Controls 设置行内字体节奏

- 当前总体迁移完成度：约 **99.65%**，仍未达到完整可玩。
- 本轮对照：
  - `D:/MDT/mindustry-upstream-v157.4/core/src/mindustry/ui/dialogs/KeybindDialog.java`
  - `rebuildBinds()` 中分类、key name、bound value 都走 Scene2D 默认 `Label` / `labelWrap` / table `add` 节奏，不应是 Rust 旧实现的 10.5px 小字。
- 本轮实现：
  - `desktop/src/lib.rs`
    - 新增 `SETTINGS_KEYBIND_CATEGORY_FONT_SIZE_LIKE_JAVA`、`SETTINGS_KEYBIND_LABEL_FONT_SIZE_LIKE_JAVA`、`SETTINGS_KEYBIND_VALUE_FONT_SIZE_LIKE_JAVA`；
    - Controls dialog 的分类行、按键名、绑定值字号统一使用 Java-like 默认 UI 字号；
    - 新增 `desktop_launcher_settings_controls_row_typography_matches_java` 回归测试，锁住字号和左对齐。
  - `README.md`
    - 迁移进度更新到 **99.65%**。
  - `MIGRATION.md`
    - 新增 `1188. 修正 Controls 设置行内字体节奏`。
- 已验证：
  - `cargo fmt`
  - `cargo test -p mindustry-desktop desktop_launcher_settings_controls_row_typography_matches_java -- --nocapture`
  - `cargo test -p mindustry-desktop settings_keybind -- --nocapture`
- 下一步优先级：
  1. Settings/Language：继续对齐 Language scrollpane 极限尺寸、row hover/check 与字体 glyph seed；
  2. Mods Browser：继续收口 row 80f / icon 64f / 8f pad 和 selection detail 按钮排布；
  3. 全局扫 raw key / 英文 fallback / 图标 token 泄漏，避免语言/字体可见缺口。
...
- 本轮实现：
  ...
  - `README.md`
    - 迁移进度更新到 **99.52%**。
  - `MIGRATION.md`
    - 新增 `1175. 清理 CustomRulesDialog 调试类名叠字`。
- 验证：
  - `cargo fmt`
...
## 上一闭环：移除通用 route-shell upstream 调试叠字
...
- 本轮对照：
  - Java `SettingsMenuDialog` / `LanguageDialog` 不显示 `upstream: SettingsMenuDialog` 这类 Rust route-shell 调试文案；
  - `LanguageDialog` ScrollPane clipped 区域应只包含 locale display names，不应容忍英文类名叠字混入。
- 本轮实现：
  - `desktop/src/lib.rs`
    - `push_active_menu_route_shell(...)` 对 `DesktopMenuRoute::Settings` 跳过 upstream route debug 行；
    - LanguageDialog clipped-list 回归测试从“允许 `upstream:*` 或 locale label”收紧为“只允许 locale label”。
  - `README.md`
    - 迁移进度更新到 **99.50%**。
  - `MIGRATION.md`
    - 新增 `1173. 清理 Settings/LanguageDialog upstream 调试行`。
- 验证：
  - `cargo fmt`
  - `cargo test -p mindustry-desktop desktop_launcher_language_dialog_keeps_400x50_rows_and_margin_24_stable_across_window_sizes_and_avoids_page_number_text --lib -- --test-threads=1 --nocapture`
  - `cargo build -p mindustry-desktop --features opengl-native-runtime`
- 下一步建议继续：
  1. 继续补 LanguageDialog / MapLocalesDialog 中 Java `flatTogglet`、outlineLabel、filterStyle 的可测视觉语义；
...
## 上一闭环：清理 PlanetDialog upstream 调试行

- 当前总体迁移完成度：约 **99.49%**，仍未达到完整可玩。
- 本轮对照：
...
- 本轮实现：
  ...
  - `README.md`
    - 迁移进度更新到 **99.49%**。
  - `MIGRATION.md`
    - 新增 `1172. 清理 PlanetDialog upstream 调试行`。
- 验证：
  - `cargo fmt`
...
## 上一闭环：清理 DatabaseDialog upstream 调试行

- 当前总体迁移完成度：约 **99.48%**，仍未达到完整可玩。
- 本轮对照：
  - Java `DatabaseDialog` 不显示 `upstream: DatabaseDialog` 这类 Rust route-shell 调试文案；
  - 数据库 route 应只显示原版搜索、tab、content grid、content info 等本地化/图标内容。
- 本轮实现：
  - `desktop/src/lib.rs`
    - `push_active_menu_route_shell(...)` 对 `DesktopMenuRoute::Database` 跳过 upstream route debug 行；
    - `database_route_lines()` 的分类行从 Rust-only `category/count/sample` 诊断串收口为纯 `@database-category.*` / `@database-tag.*` key；
    - Database route 回归测试改为断言不渲染 `upstream: DatabaseDialog`，并检查分类标题与 ContentInfo 统计 label 已走本地化输出。
  - `README.md`
    - 迁移进度更新到 **99.48%**。
  - `MIGRATION.md`
    - 新增 `1171. 清理 DatabaseDialog upstream 调试行`。
- 验证：
  - `cargo fmt`
  - `cargo test -p mindustry-desktop desktop_launcher_menu_sub_action_routes_to_database_dialog_shell --lib -- --test-threads=1 --nocapture`
  - `cargo build -p mindustry-desktop --features opengl-native-runtime`
- 下一步建议继续：
  1. 继续按 route 清理 Planet 等剩余 upstream 调试行；
  2. 继续补 LanguageDialog / MapLocalesDialog 中 Java `flatTogglet`、outlineLabel、filterStyle 的可测视觉语义；
  3. Settings/Data 与字体/语言可见层继续隔离 raw diagnostics、英文 token 和 fallback 泄漏。

## 上一闭环：补齐 IconLarge 字体 glyph seed
...
- 本轮对照：
  - Java `LoadDialog` / `SaveDialog` 不显示 `upstream: LoadDialog` 或 `upstream: SaveDialog` 这类 Rust route-shell 调试文案；
  - 暂停菜单进入保存/读取后应只显示原版存档列表、搜索、模式筛选、`@save.new` 等本地化控件。
- 本轮实现：
  - `desktop/src/lib.rs`
    - `push_active_menu_route_shell(...)` 对 `DesktopMenuRoute::LoadGame | SaveGame` 跳过 upstream route debug 行；
    - 暂停菜单进入 SaveGame 的回归测试改为断言不渲染 `upstream: SaveDialog`。
  - `README.md`
    - 迁移进度更新到 **99.46%**。
  - `MIGRATION.md`
    - 新增 `1169. 清理存取档对话框 upstream 调试行`。
- 验证：
  - `cargo fmt`
...
## 上一闭环：清理 PausedDialog upstream 调试行

- 当前总体迁移完成度：约 **99.45%**，仍未达到完整可玩。
- 本轮对照：
...
- 本轮实现：
  ...
  - `README.md`
    - 迁移进度更新到 **99.45%**。
  - `MIGRATION.md`
    - 新增 `1168. 清理 PausedDialog upstream 调试行`。
- 验证：
  - `cargo fmt`
  - `cargo test -p mindustry-desktop desktop_launcher_paused_world_overlay_renders_save_and_load_entries --lib -- --test-threads=1 --nocapture`
  - `cargo build -p mindustry-desktop --features opengl-native-runtime`
...
## 上一闭环：清理 AboutDialog upstream 调试行

- 当前总体迁移完成度：约 **99.44%**，仍未达到完整可玩。
- 本轮对照：
  - Java `AboutDialog` 不显示 `upstream: AboutDialog` 这类 Rust route-shell 调试文案；
  - About/Credits/Links 路径应只显示 Java 对话框自身标题、credits 按钮、link cards、contributors 和本地化文案。
- 本轮实现：
  - `desktop/src/lib.rs`
    - `push_active_menu_route_shell(...)` 对 `DesktopMenuRoute::About` 跳过 upstream route debug 行；
    - AboutDialog 回归测试改为断言不渲染 `upstream: AboutDialog`，避免继续把类名调试文本固化为可见 UI。
  - `README.md`
    - 迁移进度更新到 **99.44%**。
  - `MIGRATION.md`
    - 新增 `1167. 清理 AboutDialog upstream 调试行`。
- 验证：
  - `cargo fmt`
  - `cargo test -p mindustry-desktop desktop_launcher_about_menu_route_renders_upstream_credits_links_and_contributors --lib -- --test-threads=1 --nocapture`
  - `cargo build -p mindustry-desktop --features opengl-native-runtime`
- 下一步建议继续：
  1. 继续按 route 清理 Paused/Save/Database 等剩余 upstream 调试行；
  2. JoinDialog Add/Edit 子弹窗继续细化 field padding 与 BaseDialog chrome；
  3. Settings/Data 与字体/语言可见层继续隔离 raw diagnostics、英文 token 和 fallback 泄漏。

## 上一闭环：清理 HostDialog upstream 调试行
...
- 本轮对照：
  - `D:/MDT/mindustry-upstream-v157.4/core/src/mindustry/ui/dialogs/JoinDialog.java`：删除保存服务器时使用 `ui.showConfirm("@confirm", "@server.delete", ...)`；
  - 社区服务器免责声明使用 `ui.showCustomConfirm("@warning", "@servers.disclaimer", "@ok", "@back", ...)`；
  - 两者正文都是纯 bundle 文案，不拼接服务器地址或 target。
- 本轮实现：
  - `desktop/src/lib.rs`
    - `push_join_delete_dialog(...)` 正文改为纯 `@server.delete`；
    - `push_join_server_disclaimer_dialog(...)` 正文改为纯 `@servers.disclaimer`；
    - Join route diagnostic line 同步去掉 `index=` / `target=`；
    - 更新删除确认和免责声明测试，断言不再出现 `| example.org` / `community-eu.example` 拼接文案。
  - `README.md`
    - 迁移进度更新到 **99.42%**。
  - `MIGRATION.md`
    - 新增 `1165. 收口 JoinDialog 确认弹窗正文`。
- 验证：
  - `cargo fmt`
...
- 本轮对照：
  - `D:/MDT/mindustry-upstream-v157.4/core/src/mindustry/ui/dialogs/LaunchLoadoutDialog.java:167-201`：候选 loadout 使用 `new SchematicImage(s)` + `Styles.togglet`；
  - `D:/MDT/mindustry-upstream-v157.4/core/src/mindustry/ui/dialogs/SchematicsDialog.java:684-745`：`SchematicImage` 绘制 `schematic-background`、preview/refresh 和边框。
- 本轮实现：
  - `desktop/src/lib.rs`
    - Campaign LaunchLoadout candidate 模型携带 schematic 尺寸和 preview tiles；
    - picker 候选项改为 `togglet` checked 底图 + `schematic-background` + 真实 schematic tile preview；
    - 删除候选项主视觉里的 core icon + display label 依赖，测试改为验证 SchematicImage 背景/tiles 与第五候选 hit-test/提交。
  - `README.md`
    - 迁移进度更新到 **99.32%**。
  - `MIGRATION.md`
    - 新增 `1155. 接入 Campaign LaunchLoadout SchematicImage 预览`。
- 验证：
  - 已通过本轮 3 个 Campaign LaunchLoadout 定向测试；
  - 已通过 `cargo build -p mindustry-desktop --features opengl-native-runtime`；
  - 已执行 `cargo fmt`。
- 下一步建议继续：
  1. LaunchLoadout picker 几何继续扩展到 Java `size(200f)` 方卡/滚动；
  2. Controls 子菜单未知 keybind raw fallback 收紧；
  3. SectorSelectDialog 完整滚动结果与 Settings/Language/Controls 字体继续收口。

## 上一闭环：接入 Campaign locked hoverLabel canSelect 分支

- 当前总体迁移完成度：约 **99.31%**，仍未达到完整可玩。
- 本轮对照：
  - `D:/MDT/mindustry-upstream-v157.4/core/src/mindustry/ui/dialogs/PlanetDialog.java:414-435`：`canSelect(Sector)`；
  - `D:/MDT/mindustry-upstream-v157.4/core/src/mindustry/ui/dialogs/PlanetDialog.java:952-973`：hoverLabel locked/cancel/selectable 文案分支。
- 本轮实现：
  - `desktop/src/lib.rs`
    - 新增 Campaign sector runtime lookup 与 `campaign_sector_can_select_like_java(...)`；
    - hoverLabel 从固定 `[accent][[ [white]name[accent] ]` 改为 Java-like locked/cancel/selectable 分支；
    - locked sector 显示本地化 `[gray]Locked`，PlanetLaunch blocked sector 使用 `Iconc.cancel` glyph + `RenderFontId::Icon`。
  - `README.md`
    - 迁移进度更新到 **99.31%**。
  - `MIGRATION.md`
    - 新增 `1154. 接入 Campaign locked hoverLabel canSelect 分支`。
- 验证：
  - 已通过本轮 3 个 Campaign UI 定向测试；
  - 已通过 `cargo build -p mindustry-desktop --features opengl-native-runtime`；
  - 已执行 `cargo fmt`。
- 下一步建议继续：
  1. LaunchLoadout 候选项接入真实 `SchematicImage` 预览；
  2. Controls 子菜单未知 keybind raw fallback 收紧；
  3. SectorSelectDialog 完整滚动结果与 Settings/Language/Controls 字体继续收口。

## 上一闭环：收口 MapPlay help 弹窗 ScrollPane 与正文字号

- 当前总体迁移完成度：约 **99.30%**，仍未达到完整可玩。
- 本轮对照：
  - `D:/MDT/mindustry-upstream-v157.4/core/src/mindustry/ui/dialogs/MapPlayDialog.java:97-113`：`displayGameModeHelp()` 使用 `BaseDialog`、`ScrollPane(table)`、`labelWrap(...).width(400f)` 与 `@ok` 按钮 `size(110, 50).pad(10f)`。
- 本轮实现：
  - `desktop/src/lib.rs`
    - 移除 help 内容区 Rust-only `button` 背景和手工描边；
    - help 正文字号调整到 `12.0`，继续保留 400 宽 wrap；
    - 扩展 MapPlay help 测试，锁定 ScrollPane 无额外 stroke、OK 按钮 `110x50`。
  - `README.md`
    - 迁移进度更新到 **99.30%**。
  - `MIGRATION.md`
    - 新增 `1153. 收口 MapPlay help 弹窗 ScrollPane 与正文字号`。
- 验证：
  - 已通过本轮 3 个定向 MapPlay UI 测试；
  - 已执行 `cargo fmt`。
- 下一步建议继续：
  1. 继续补 Campaign locked hover text 与 SectorSelectDialog 完整滚动结果；
  2. 继续对照 Java LaunchLoadout 的 `SchematicImage` 视觉；
  3. 继续精修 Settings/Language/Controls 字体与所有子菜单视觉。

## 上一闭环：收口 UI 图标字体、toggle 状态与数据页 chrome

- 当前总体迁移完成度：约 **99.29%**，仍未达到完整可玩。
- 本轮对照：
  - `SectorSelectDialog.java` 搜索控件使用 `Icon.zoom` 等 icon font；
  - `LaunchLoadoutDialog.java` 的 `@resources.max` 使用 `Styles.togglet` checked 状态；
  - `ResearchDialog.java` 的 progress 使用 `research.progress` bundle format；
  - `SettingsMenuDialog.java` 的 Data/PlanetData table 背景不额外叠 Rust-only stroke。
- 本轮实现：
  - `desktop/src/lib.rs`
    - Campaign sector list 搜索/折叠/清除改用 Java icon font glyph；
    - 新增 checked text button seam，并把 LaunchLoadout `@resources.max` 改为 `togglet` checked；
    - TechTree progress 改为 `research.progress` bundle format；
    - 删除 Settings Data/PlanetData 容器额外 stroke。
  - `core/assets/icons/icons.properties`
    - 与上游 LF 字节级对齐，减少字体/图标资源核对噪声。
  - `README.md`
    - 迁移进度更新到 **99.29%**。
  - `MIGRATION.md`
    - 新增 `1152. 收口 UI 图标字体、toggle 状态与数据页 chrome`。
- 验证：
  - 已通过本轮 4 个定向 UI 测试；
  - 已执行 `cargo fmt`。
- 下一步建议继续：
...
## 上一闭环：收口 Campaign LaunchLoadout 候选卡片 debug 文案
...
- 本轮对照：
  - `D:/MDT/mindustry-upstream-v157.4/core/src/mindustry/editor/MapLocalesDialog.java:384-404`：`propEditDialog(...)` 中 `@locales.addicon` 打开 `addIconDialog(...)` 并追加图标输出；
  - `D:/MDT/mindustry-upstream-v157.4/core/src/mindustry/editor/MapLocalesDialog.java:619-706`：`addIconDialog(...)`/`iconsTable(...)` 搜索 Iconc/content/team 候选，按钮 48f、cell 52f、最多 20 列。
- 本轮实现：
  - `desktop/src/lib.rs`
    - 新增 `editor_map_locales_add_icon_*` 状态与 `Open/Close/Focus/Clear/AddEditorLocalesIcon` action；
    - `@locales.addicon` 改为真实可点击，打开 Java-like addIcon chooser；
    - 候选接入当前 content registry、content icon glyph registry、texture atlas 与 base team emoji；
    - chooser 支持搜索、clear、滚轮、tooltip、点击候选后追加到 selected locale 当前属性值并关闭。
  - `README.md`
    - 迁移进度更新到 **99.18%**。
  - `MIGRATION.md`
    - 新增 `1142. 接入 MapLocales addIconDialog 桌面真 UI`。
- 验证：
  - `cargo fmt --all`
...
- 下一步建议继续：
  1. About links 的 `link.*.title`/description bundle 解析按 Java `Links.java` 收口；
...
- 本轮对照：
  - `D:/MDT/mindustry-upstream-v157.4/core/src/mindustry/ui/dialogs/ModsDialog.java:551-555`：browser selection 使用 `BaseDialog(mod.name)`，正文进入 `sel.cont.pane(...)`，文本 `labelWrap(...).width(mobile ? 400f : 500f).pad(4f).labelAlign(Align.center, Align.left)`，按钮保持 `150f x 54f`。
- 本轮实现：
  - `desktop/src/lib.rs`
    - 新增 `mods_browser_selection_details_pane_rect_for_browser(...)`，selection 描述/作者文本进入 Java 式 pane 区域；
    - `push_mods_browser_selection_dialog(...)` 按桌面 500f、portrait 400f 计算 wrap width，文本 top-align 到 pane 内 4f padding；
    - 扩展 selection dialog 回归测试，锁定 pane、wrap、top align 与按钮尺寸。
  - `README.md`
    - 迁移进度更新到 **99.15%**。
  - `MIGRATION.md`
    - 新增 `1139. 收口 ModsBrowser selection 详情 pane 节奏`。
- 验证：
  - `cargo fmt --all`
  - `cargo test -p mindustry-desktop desktop_launcher_mods_browser_selection_dialog_renders_details_and_buttons --lib -- --nocapture`
  - `cargo test -p mindustry-desktop desktop_launcher_mods_browser_selection_dialog_omits_missing_author_and_description_placeholders_like_java --lib -- --nocapture`
  - `cargo test -p mindustry-desktop desktop_launcher_load_game_route_toggles_mode_filters_like_upstream_load_dialog --lib -- --nocapture`
  - `git diff --check`
- 下一步建议继续：
  1. ModsBrowser installed detail、release rows 与 View Content tooltip 继续按 Java `ModsDialog` 收口；
  2. Settings 子页面按钮字号、图标尺寸、语言默认 locale 物化继续按 Java `SettingsMenuDialog` / `LanguageDialog` 收口；
  3. Menu chrome 的 `discordBanner` / `infoBanner` / mobile gutter 继续接入真实菜单渲染命令；
  4. 完整可玩与 Java↔Rust 联机兼容仍需推进，不能宣告目标完成。

## 上一闭环：收口 Load/Save 模式过滤 tooltip chrome
...
- 本轮对照：
  - `D:/MDT/mindustry-upstream-v157.4/core/src/mindustry/ui/Fonts.java`：内容图标只有 atlas region 在 UI texture page 才注册进字体；
  - `D:/MDT/mindustry-upstream-v157.4/core/src/mindustry/core/UI.java`：`:token:` 只替换 built-in `Iconc.codes` 或已注册 `Fonts.hasUnicodeStr(token)`。
- 本轮实现：
  - `desktop/src/lib.rs`
    - `desktop_content_icon_glyph_registerable_like_java(...)` 改为只认 UI page resolved 图标；
    - `format_icons_like_java(...)` 的内容图标替换也通过 registerable gate；
    - `DesktopFontRasterizationPlan` 增加独立 `missing_icon_atlas_symbols` 诊断，missing atlas 不再进入 content icon glyph 计划；
    - 更新相关 token、font plan、team emoji 和 upload 回归测试。
  - `README.md`
    - 迁移进度更新到 **99.07%**。
  - `MIGRATION.md`
    - 新增 `1133. 收紧内容图标字体注册与 token 替换边界`。
- 验证：
  - `cargo fmt --all`
...
## 上一闭环：收口 LanguageDialog 默认 locale 物化语义
...
- 本轮对照：
  - `D:/MDT/mindustry-upstream-v157.4/core/src/mindustry/ui/dialogs/ModsDialog.java:552-554`：browser selection 直接拼 `mod.description` / `mod.author`；
  - `D:/MDT/mindustry-upstream-v157.4/core/src/mindustry/core/UI.java:499-531`：错误正文只在开头 `@key` 场景做 bundle 查找，中间 raw `@token` 不应被误翻译。
- 本轮实现：
  - `desktop/src/lib.rs`
    - ModsBrowser selection 缺 description 时不再显示 `@none` / `None`；
    - ModsBrowser selection 缺 author 时不再显示 `@unknown` / `Unknown`；
    - 新增 `localize_leading_bundle_key_text_like_java(...)`，只本地化错误文本开头的 `@bundle.key`，保留中间 raw `@token`；
    - LoadGame error、Mods GitHub import error、Mods directory error 弹窗统一切到 Java 式 leading-key 语义。
  - `README.md`
    - 迁移进度更新到 **99.05%**。
  - `MIGRATION.md`
    - 新增 `1131. 收口 ModsDialog 缺值占位与错误文本 raw bundle 语义`。
- 验证：
  - `cargo fmt --all`
  ...
  - `git diff --check`
- 下一步建议继续：
  1. Settings/Language：`default` locale 打开语言页时物化为 closest locale，`LANGUAGE` 环境变量不应覆盖 Java `Locale.getDefault()` 路径；
  2. Settings 子页通用按钮字号、图标字号、icon/label 间距按 Java `TextButtonStyle` 收口；
...
## 上一闭环：收口 JoinDialog 动态描述与 modeName 本地化语义
...
- 本轮对照 `D:/MDT/mindustry-upstream-v157.4/core/src/mindustry/editor/MapLocalesDialog.java`：
  - 主界面 `Icon.filter` 应打开 `filterDialog(this::buildMain)`，不是直接切换 by-name/by-value；
  - property view 顶部 `Icon.filter` 应打开同一个 `filterDialog(() -> buildPropView(key))`；
  - filterDialog 是 `BaseDialog("@locales.filter")`，包含 `@search`、`@locales.byname/byvalue`、`@locales.showcorrect/showmissing/showsame` 与 `@back`。
- 本轮实现：
  - `desktop/src/lib.rs`
    - 新增 `editor_map_locales_filter_open` 桌面状态；
    - 新增打开/关闭 filterDialog、设置 search-by-value、切换 show correct/missing/same 的 action 与 dispatch；
    - 主 MapLocales 与 property view 的 filter 图标改为打开 Java 式独立子弹窗；
    - filterDialog 用 BaseDialog shell、`Styles.togglet` 搜索模式按钮、`whitePane + Pal.gray/accent/techBlue` 状态按钮与 ok/cancel 图标；
    - Back/Esc 优先关闭 filterDialog，并保持 property view/main dialog 栈不被误关闭；
    - 回归测试覆盖主 dialog 与 property view 打开 filterDialog、切换 by-value、切换 show-missing、关闭后继续编辑。
  - `README.md`
    - 迁移进度更新到 **99.03%**。
  - `MIGRATION.md`
    - 新增 `1129. 接入 MapLocales filterDialog 子弹窗`。
- 验证：
  - `cargo fmt --all`
...
- 下一步建议继续：
  ...
  2. JoinDialog 动态描述、服务端 `modeName`、ModsDialog displayName/缺作者、错误文本原样显示等语言/字体缺口优先收口；
  3. Settings / Join / Database / ContentInfo / Host / Load 等用户可见子菜单继续按 Java 原版视觉和默认字体节奏对齐；
  4. 完整可玩与 Java↔Rust 联机兼容仍需推进，不能宣告目标完成。

## 上一闭环：收口 ModsBrowser 子弹窗字体与动态语言值

- 当前总体迁移完成度：约 **99.02%**，仍未达到完整可玩。
- 本轮对照 `D:/MDT/mindustry-upstream-v157.4/core/src/mindustry/ui/dialogs/ModsDialog.java`：
  - selection/release 子弹窗应继承 `BaseDialog` shell；
  - selection 正文为 description + 本地化 `editor.author` 标签 + 原始 author 值；
  - release row 使用 `Tex.whiteui` tint `Pal.darkestGray`，标题/日期保持默认 label 节奏；
  - detail 与 selection 的 author/version/description 是动态值，Java 只本地化标签，不把动态值当 bundle key。
- 本轮实现：
  - `desktop/src/lib.rs`
    - ModsBrowser selection/release 子弹窗改用 `BaseDialog` shell；
    - selection/release 文本字号回到 `SETTINGS_JAVA_DEFAULT_FONT_SIZE` 并去掉额外小字号 outline/stroke；
    - release row 背景改为 `whiteui + Pal.darkestGray`；
    - author/version 动态值停止误走 `localize_bundle_markup_text(...)`；
    - 新增 `desktop_launcher_mods_dynamic_values_remain_raw_like_java_showmod`。
  - `README.md`
    - 迁移进度更新到 **99.02%**。
  - `MIGRATION.md`
    - 新增 `1128. 收口 ModsBrowser 子弹窗字体与动态语言值`。
- 验证：
  - `cargo fmt --all`
...
- 下一步建议继续：
  ...
  2. 主菜单 `discordBanner` / `infoBanner` / mobile gutter / `noBarPane` 继续补齐；
  3. Host / Join 与 Database/ContentInfo 语言、字体、动态描述链继续收口；
  4. 完整可玩与 Java↔Rust 联机兼容仍需推进，不能宣告目标完成。

## 上一闭环：接入 MapLocales property view 子菜单

- 当前总体迁移完成度：约 **99.01%**，仍未达到完整可玩。
- 本轮对照 `D:/MDT/mindustry-upstream-v157.4/core/src/mindustry/editor/MapLocalesDialog.java`：
  - `propEditDialog(...)` 由 property card 的 edit 图标打开；
  - `@locales.viewproperty` 打开 `viewPropertyDialog(key)`；
  - property view 标题使用 `Core.bundle.format("locales.viewing", key)`；
  - view 顶部是 filter + TextField + clear，主体是按 locale 渲染的 property cards，missing 行可 `@add` 后编辑。
- 本轮实现：
  - `desktop/src/lib.rs`
    - 新增 MapLocales property edit / property view 状态、action、hit-test、Back/Esc 关闭顺序；
    - 主 property card edit 图标接入 `OpenEditorLocalesPropertyEdit`；
    - property edit 弹窗渲染 `@locales.addtoother` 与 `@locales.viewproperty` 等按钮；
    - property view 弹窗渲染 search/filter/clear、locale displayName、状态色、value TextField、missing add；
    - missing add 与 value 文本输入/Backspace/Delete 直接修改真实 `MapLocalesDialog` 数据；
    - 新增 `desktop_launcher_editor_map_locales_property_view_dialog_renders_and_edits_like_java`。
  - `README.md`
    - 迁移进度更新到 **99.01%**。
  - `MIGRATION.md`
    - 新增 `1127. 接入 MapLocales property view 子菜单`。
- 验证：
  - `cargo fmt --all`
...
- 下一步建议继续：
  ...
  2. ModsBrowser selection / release / detail 子弹窗继续按 Java `ModsDialog` 收口；
  3. Host / Join 与主菜单 skin 资源层继续按 Java 原版截图/command diff 收口；
  4. 完整可玩与 Java↔Rust 联机兼容仍需推进，不能宣告目标完成。

## 上一闭环：收口 ModsBrowser 搜索排序与卡片视觉节奏
...
- 本轮对照 `D:/MDT/mindustry-upstream-v157.4/core/src/mindustry/ui/dialogs/LoadDialog.java`：
  - 存档卡片是 `new TextButton("", Styles.grayt)`，卡片背景由 `Styles.grayt` 提供；
  - 预览图仍由 `BorderImage(def, 4f)` 包边；
  - Java 没有给整张存档卡片额外画一圈 Rust-only `StrokeRect`。
- 本轮实现：
  - `desktop/src/lib.rs`
    - 删除 `push_load_game_route_page(...)` 中每张存档卡片额外的整卡 `stroke_rect(rect, ...)`；
...
## 上二闭环：接入 LanguageDialog core 模型
...
- 本轮对照 `D:/MDT/mindustry-upstream-v157.4/core/src/mindustry/ui/dialogs/LanguageDialog.java`：
  ...
  - 每个语言按钮 `size(400f, 50f)`；
  - 点击不同 locale 后写设置并 `ui.showInfo("@language.restart")`；
  - `findClosestLocale()` 先 exact，再 same language，最后 `en`。
- 本轮实现：
  - `core/src/mindustry/ui/dialogs/language_dialog.rs`
    - 新增 `LanguageDialog` / `LanguageDialogLocale` / `LanguageDialogRow`；
    - 固定导出 `LANGUAGE_DIALOG_ROW_WIDTH = 400.0`、`LANGUAGE_DIALOG_ROW_HEIGHT = 50.0`、`LANGUAGE_DIALOG_TABLE_MARGIN_HORIZONTAL = 24.0`、`LANGUAGE_DIALOG_RESTART_MESSAGE_KEY = "@language.restart"`；
    - 新增 core 测试覆盖按钮 metrics、restart notice、closest locale 顺序。
  - `core/src/mindustry/ui/dialogs/mod.rs`
    - 导出 LanguageDialog 模型和常量。
  - `desktop/src/lib.rs`
    - Settings/Language 的行高、宽度、左右 margin、restart key 改为引用 core LanguageDialog 常量；
    - 新增 `settings_language_dialog_model_like_java()`；
    - `push_settings_language_dialog_content(...)` 改为从 `LanguageDialog::rows()` 读取 display/selected/metric，再由 desktop 负责 ScrollPane 裁剪与绘制。
- 验证：
  ...
  - `cargo fmt --all`
- 下一步建议继续：
  1. Load/Save 子菜单视觉：搜索条、filter chip、存档卡片、操作图标和点击穿透；
...
- 本轮实现：
  - `desktop/src/lib.rs`
    - 新增/接线 `MAP_LOCALES_BELARUSIAN_DISPLAY_NAMES`、`MAP_LOCALES_BULGARIAN_DISPLAY_NAMES`、`MAP_LOCALES_CATALAN_DISPLAY_NAMES`、`MAP_LOCALES_ESTONIAN_DISPLAY_NAMES`、`MAP_LOCALES_BASQUE_DISPLAY_NAMES`、`MAP_LOCALES_FILIPINO_DISPLAY_NAMES`、`MAP_LOCALES_LITHUANIAN_DISPLAY_NAMES`、`MAP_LOCALES_SERBIAN_DISPLAY_NAMES`、`MAP_LOCALES_THAI_DISPLAY_NAMES`、`MAP_LOCALES_TURKMEN_DISPLAY_NAMES`、`MAP_LOCALES_UKRAINIAN_DISPLAY_NAMES`；
...
- 验证：
  ...
  - `git diff --check`
- 下一步建议继续：
  1. 前端/字体优先：LanguageDialog 连续 ScrollPane、400x50 行按钮、24px margin、重启提示与 Java 命中一致；
  2. Settings/Language 字号、默认字体 shadow、outline 预烘焙字形，不要出现额外 runtime outline；
  3. 继续审查真实字体 atlas 对新增变音符/非拉丁字形的渲染质量；
  4. Load/Save、Join、CustomGame、Database/ContentInfo 子菜单视觉继续按 Java 源码收口。

## 更早闭环：MapLocales 补齐捷克丹麦芬兰匈牙利罗马尼亚瑞典显示名
...
- 验证：
  ...
  - `git diff --check`
- 仍未完成：
  - 当时仍未覆盖 `be/bg/ca/et/eu/fil/lt/sr/th/tk/uk_UA`，已由最新闭环补齐；
...
## 更早闭环：JoinDialog 卡片文本 wrap 与 ellipsis 收口
...
- 本轮实现：
  - `desktop/src/lib.rs`
    - 用本机 JDK 17 生成 `vi/nl/nl_BE` 的 Java display-name 表；
    - 新增 `MAP_LOCALES_VIETNAMESE_DISPLAY_NAMES`、`MAP_LOCALES_DUTCH_DISPLAY_NAMES`、`MAP_LOCALES_DUTCH_BELGIUM_DISPLAY_NAMES`；
    - `map_locales_display_names_for_ui_locale(...)` 接入 `vi` 和 `nl/nl_BE`；
    - 新增表进入 `desktop_extend_font_seed_characters_from_map_locale_display_names(...)`；
    - 扩展 MapLocales 显示名与字体 seed 测试。
- 验证：
...
- 本轮对照 `D:/MDT/mindustry-upstream-v157.4/core/src/mindustry/ui/dialogs/JoinDialog.java`：
  - `setupRemote()` 只给真实 saved server 建卡；
  - `@host.invalid` 只用于真实 saved server 刷新失败 body，不用于 saved 列表为空的占位卡。
- 本轮实现：
  - `push_join_route_page(...)` 移除 saved servers 为空时额外 `pane + @host.invalid` fallback；
  - 新增 `desktop_launcher_join_route_empty_saved_section_does_not_render_placeholder_like_java`；
  - 验证 `join_route` 时修复 `last_connect_packet_error` modal detail：保留 `Connection error.` + `send failed`，但 reconnect fallback 仍不暴露 Rust-only ping fallback。
- 验证：
  ...
  - `git diff --check`
- 下一步建议继续前端：
  1. 主菜单桌面态：按钮状态、黑底面板、子菜单间距/淡入淡出；
  2. Settings：字体/图标尺寸/主行 chrome 与 Java `Styles.flatt`、`Vars.iconMed` 进一步收口；
  3. Load/Content/Join 列表卡片密度、scrollbar pressed/drag 与 card shell 继续逐项对齐。

## 上一闭环：LanguageDialog 裁剪外行不再响应 hover/click
...
- 当前总体迁移完成度：约 **86.2%**，仍未达到完整可玩。
- 本轮对照 `D:/MDT/mindustry-upstream-v157.4/core/src/mindustry/ui/dialogs/JoinDialog.java` 与 `core/src/mindustry/ui/fragments/LoadingFragment.java`：
  - Java `connect(...)` 进入 `ui.loadfrag.show("@connecting")`；
  - Java `setButton(...)` 显示取消按钮，取消时 `disconnectQuietly()`；
  - Rust 现在由 `join_connecting_loadfrag_visible()` 显示全屏 `@connecting` 黑幕、WarningBar、spinner 与 `@cancel` 按钮；
  - input 侧会优先吞掉底层 Join/主菜单点击，`@cancel` 与 back/escape 都会触发 `CancelJoinConnect`。
- 验证：
  - `cargo fmt --all`
...
## 上一闭环：JoinDialog 版本不匹配改为 showInfo 风格 modal
...
- 本轮对照 `D:/MDT/mindustry-upstream-v157.4/core/src/mindustry/ui/dialogs/KeybindDialog.java`：
  - Java `rebuildBinds()` 在 `KeyBind.all` 循环之后追加 `@settings.reset`；
  - `bindsTable` 整体在 `ScrollPane` 内，所以 Reset All 是滚动内容最后一行，不是固定 footer；
  - Rust 现在把 Reset All 作为 `filtered_specs.len() + 1` 的虚拟尾行参与渲染、滚动 offset 和 hit-test。
- 验证：
  - `cargo fmt --all`
...
- 当前总体迁移完成度：约 **85.9%**，仍未达到完整可玩。
- 本轮对照 `D:/MDT/mindustry-upstream-v157.4/core/src/mindustry/ui/dialogs/KeybindDialog.java`：
  - Java `rebuildBinds()` 搜索只比较 `keybind.<name>.name` 的显示文本；
  - Rust `settings_keybind_filtered_specs()` 现在去掉 `trim()`、内部 `spec.name` 匹配与 `category` 匹配，仅按 `settings_keybind_bundle_label(spec.name)` 过滤；
  - 回归测试用 `general` 锁定：分类名不再触发 `@category.general.name` / `@keybind.move_x.name` 显示。
- 验证：
  - `cargo fmt --all`
  - `cargo test -p mindustry-desktop desktop_launcher_settings_route_uses_structured_settings_menu_dialog_shell --lib -- --test-threads=1 --nocapture`
  - `cargo test -p mindustry-desktop settings --lib -- --test-threads=1 --nocapture`
  ...
  - `git diff --check`
- 下一步建议继续前端：
  1. Controls `Reset All` 继续从固定 footer 语义收口为 ScrollPane 内容末尾行；
  2. JoinDialog 连接中/重连中补 Java `loadfrag.show("@connecting")` / `@reconnecting` 同款遮罩与取消按钮；
  3. 版本不匹配从 status bar/top info 收口成更像 Java `ui.showInfo(...)` 的 modal。

## 上一闭环：LoadDialog loading 提升为全屏 LoadingFragment 遮罩
...
- 本轮对照 `D:/MDT/mindustry-upstream-v157.4/core/src/mindustry/ui/dialogs/LoadDialog.java`：
  - Java 在缺失预览时使用 `Core.atlas.find("nomap")`；
  - Rust 现在缺失存档预览时只渲染 `nomap` drawable，不再在预览中心显示 `save_slot_<index>` 内部纹理 key；
  - 回归测试已锁定：缺失预览仍有 `nomap`，且不会暴露内部 preview texture key。
- 验证：
  - `cargo fmt --all`
  - `cargo test -p mindustry-desktop desktop_launcher_load_game_route_lists_save_slots_and_records_slot_click --lib -- --nocapture`
...
- 下一步建议继续前端：
  1. LoadDialog loading 罩层继续向 Java `ui.loadAnd(...)` 的全屏退场/黑屏过渡靠拢；
  2. Controls `Reset All` 高度从 `40.0` 对齐 Java `50f`；
  3. JoinDialog 免责声明/版本不匹配/连接失败弹窗层级继续收口。

## 上一闭环：SettingsMenuDialog mobile 条件可见性

- 当前总体迁移完成度：约 **85.4%**，仍未达到完整可玩。
- 本轮对照 `D:/MDT/mindustry-upstream-v157.4/core/src/mindustry/ui/dialogs/SettingsMenuDialog.java`：
  - Java `rebuildMenu()` 仅在 `!mobile || Core.settings.getBool("keyboard")` 时显示 `@settings.controls`；
  - Rust 现在用可见 entry 列表统一驱动 settings 主菜单渲染、route-lines、hit-test 与 action 映射；
  - mobile 且 keyboard=false 时 controls 隐藏，`@settings.data` 前移且命中仍进入 Data 页；
  - mobile 且 `game.keyboard=true` 或桌面端时 controls 显示并可打开 ControlsDialog。
  - Data 页新增可见 action 列表：mobile 隐藏 Java 同款 `@data.openfolder`，`@crash.export` 保持可见且 line action 映射不偏移。
- 验证：
  - `cargo fmt --all`
  ...
  - `cargo test -p mindustry-desktop settings --lib -- --nocapture`
- 下一步建议继续前端：
  1. SettingsMenuDialog `addCategory(...)` 可扩展分类和更多文本设置项；
  2. SaveDialog 保存侧 Java 原子 backup/rollback；
  3. MapPlay 极端 mode 渲染禁用态与 help/customize 层级回归。

## 上一闭环：LoadDialog runLoadSave 坏档回退与状态清理
