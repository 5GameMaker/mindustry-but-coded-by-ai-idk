...
/// Packet IDs assigned by `mindustry.net.Net.registerPacket(...)` in upstream v158.1.
pub mod packet_ids {
    use super::PacketId;
...
pub const BLOCK_SNAPSHOT_CALL_PACKET_MANIFEST: PacketManifestEntry = PacketManifestEntry {
    id: Some(packet_ids::BLOCK_SNAPSHOT_CALL_PACKET),
    name: "BlockSnapshotCallPacket",
    transport: PacketTransport::NetPacket,
    direction: PacketDirection::ServerToClient,
    streamable: false,
    ...
    force_uncompressed: false,
    codec: PacketCodecState::Implemented,
    upstream: "mindustry.gen.BlockSnapshotCallPacket",
...
pub const PACKET_MANIFEST_PHASE1_GAPS: [&str; 2] = [
    "Generated InvokePacket classes are not listed separately because upstream v158.1 does not check them in with the generated packet output.",
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct BlockSnapshotCallPacket {
    pub amount: i16,
    pub data: Vec<u8>,
}

impl PacketRuntime for BlockSnapshotCallPacket {
    fn priority(&self) -> PacketPriority {
        PacketPriority::Low
    }
...
impl PacketCodec for BlockSnapshotCallPacket {
    fn read_from<R: Read>(read: &mut R) -> std::io::Result<Self> {
        Ok(Self {
...
impl PacketRuntime for BuildHealthUpdateCallPacket {
    fn priority(&self) -> PacketPriority {
...
impl PacketCodec for BuildHealthUpdateCallPacket {
    fn read_from<R: Read>(read: &mut R) -> std::io::Result<Self> {
        Ok(Self {
            buildings: read_int_seq(read)?,
        })
    }
...
mod tests {
    ...
    fn generated_simple_call_packets_use_typeio_field_order() {
        ...
        let block_snapshot = BlockSnapshotCallPacket {
            amount: 21,
            data: vec![1, 2, 3],
        };
        let mut bytes = Vec::new();
        ...
        let mut expected = Vec::new();
        expected.extend_from_slice(&21i16.to_be_bytes());
        expected.extend_from_slice(&3i16.to_be_bytes());
        expected.extend_from_slice(&[1, 2, 3]);
        assert_eq!(bytes, expected);
        assert_eq!(
            BlockSnapshotCallPacket::read_from(&mut bytes.as_slice()).unwrap(),
            block_snapshot
        );

        let build_health_update = BuildHealthUpdateCallPacket {
            buildings: vec![9, 8, 7],
        };
        let mut bytes = Vec::new();
        build_health_update.write_to(&mut bytes).unwrap();
        let mut expected = Vec::new();
    ...
    fn manifest_supports_lookup_by_name_and_transport_id() {
        ...
        let block_snapshot = find_packet_by_name("BlockSnapshotCallPacket").unwrap();
        assert_eq!(
            block_snapshot.id,
            Some(packet_ids::BLOCK_SNAPSHOT_CALL_PACKET)
        );
        assert_eq!(block_snapshot.direction, PacketDirection::ServerToClient);
        assert_eq!(block_snapshot.priority, Some(PacketPriority::Low));
        ...
        assert_eq!(
            build_health_update.direction,
            PacketDirection::ServerToClient
        );
        assert_eq!(build_health_update.priority, Some(PacketPriority::Normal));
        assert!(build_health_update.allow_client_endpoint);
        assert!(!build_health_update.allow_server_endpoint);
        ...
        for name in [
            ...
            let packet = find_packet_by_name(name).unwrap();
            assert_eq!(
                packet.direction,
                PacketDirection::ServerToClient,
                "{name} must follow Java @Remote default targets = Loc.server"
            );
            assert!(packet.allow_client_endpoint, "{name}");
            assert!(!packet.allow_server_endpoint, "{name}");
        }
    ...
    fn generated_v157_4_new_call_packets_roundtrip() {
        ...
        expected.extend_from_slice(&3i32.to_be_bytes());
        expected.extend_from_slice(&12i32.to_be_bytes());
        expected.extend_from_slice(&34i32.to_be_bytes());
        expected.extend_from_slice(&56i32.to_be_bytes());
        assert_eq!(bytes, expected);
        assert_eq!(
            BuildHealthUpdateCallPacket::read_from(&mut bytes.as_slice()).unwrap(),
            build_health_update
